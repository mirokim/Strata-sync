/**
 * VaultManagerTab — Vault file management and data refinement hub.
 *
 * Default view: vault selection + stats + pipeline execution + script grid
 * Advanced view: quality checklist + periodic schedule
 */
...
const QUALITY_CHECKLIST = [
  { id: 'no-link',      label: 'Unlinked file ratio',                  target: '0%',                   script: 'check_quality.py' },
  ...
  { id: 'source-url',   label: 'Missing source URL (imported files)',   target: 'None',                 script: 'check_quality.py' },
  { id: 'img-naming',   label: 'Image filename rule violations',       target: 'None',                 script: 'check_links.py' },
  { id: 'thin-file',    label: 'Files under 300 chars (standalone)',    target: 'None',                 script: 'scan_cleanup.py' },
  ...
  { id: 'chief-tag',    label: 'Missing chief tag feedback files',     target: 'None',                 script: 'gen_year_hubs.py' },
  ...
  { id: 'orphan',       label: 'Orphaned attachment files',            target: 'None',                 script: 'check_quality.py' },
]

// Periodic refinement schedule
const PERIODIC_TASKS = [
  ...
  },
  {
    period: 'Monthly',
    tasks: [
      { script: 'check_quality.py',    label: 'Check unlinked files' },
  ...
  },
  {
    ...
    tasks: [
      ...
      { script: 'check_outdated.py',   label: 'Review outdated docs for archival' },
...
export default function VaultManagerTab() {
  ...
  const runScript = async (script: ScriptDef) => {
    if (!vaultPath || isBusy) return
    setRunningScript(script.name)
    addLog(`> ${script.name}`)
    try {
      addLog('  Script execution requires Electron environment')
  ...
  const canRun = Boolean(vaultPath) && !isBusy
          ...
          <div style={{
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 2 }}>
                Graph RAG BFS entry point — recommended update within 2 weeks
              </div>
            </div>
            <div style={{
            ...
            {log.map((line, i) => (
              <div key={i} style={{
                ...
                color: line.includes('Error') || line.includes('Failed') ? 'var(--color-error)'
                  : line.includes('Done') || line.includes('complete') || line.includes('Pipeline') ? 'var(--color-success)'
      ...
      <section>
        ...
        {showAdvanced && (
          <div className="flex flex-col gap-5" style={{ marginTop: 16 }}>
            ...
            <div>
              <div style={{
                ...
                <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>
                  {checkedItems.size}/{QUALITY_CHECKLIST.length} completed
                  {checkedItems.size > 0 && (
                    <button
            ...
            {/* Graph RAG freshness bug mitigation */}
            <div style={{
              ...
              <div style={{ fontSize: 12, color: 'var(--color-text-muted)', lineHeight: 1.7, marginBottom: 10 }}>
                If the AI responds with outdated information as if it were current, run the scripts below in order.
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {[
                  { step: '1', script: 'check_outdated.py', note: 'Check outdated files and isolated new docs' },
                  { step: '2', script: 'gen_year_hubs.py',  note: 'Update year hub "Recently Added" section' },
                  { step: '3', script: 'gen_index.py',      note: 'Add latest docs to _index.md header' },
